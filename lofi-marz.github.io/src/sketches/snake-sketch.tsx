...
const opposites = [Direction.None, Direction.Down, Direction.Left, Direction.Up, Direction.Right];
...
class Snake {
    ...
    update() {
        ...
        const nextHead = {x:this.segments[0].x + dir[0], y:this.segments[0].y + dir[1]};
    ...
    draw(p5: P5, size: number) {
        ...
        for (let i = 0; i < this.segments.length; i++) {
            ...
            p5.vertex(this.segments[i].x * size + size/2, this.segments[i].y * size + size/2);
        }
        p5.endShape();
    }
...
const SnakeSketch: React.FC = () => {
    ...
    const getNewFruitPos = (p5: P5) => {
        let newPos = {x:p5.round(p5.random(CELLS-1)), y:p5.round(p5.random(CELLS-1))};
        while (snake.contains(newPos)) {
            newPos = {x:p5.round(p5.random(CELLS-1)), y:p5.round(p5.random(CELLS-1))};
        }
        return newPos;
    };
    ...
    const onGetFruit = (p5: P5) => {
        ...
        fruitPos = {x:p5.round(p5.random(CELLS-1)), y:p5.round(p5.random(CELLS-1))};
        snake.segments.push({x:lastSegment.x  - dir[0], y:lastSegment.y - dir[1]});
    };
    ...
    const draw = (p5: P5) => {
        ...
        if (timeElapsed >= UPDATE_RATE) {
            ...
            for (let i = 1; i < snake.segments.length; i++) {
                if (snake.segments[i].x == head.x && snake.segments[i].y == head.y) startGame(p5);
            }

            if (head.x < 0 || head.x >= CELLS || head.y < 0 || head.y >= CELLS) startGame(p5);
        ...
        p5.noStroke();
        p5.fill('#e03131');
        p5.circle(fruitPos.x * cellSize + cellSize/2, fruitPos.y * cellSize + cellSize/2, cellSize/2);
    };
    ...
    return <Sketch setup={setup} className='shadow' draw={draw} keyPressed={onKeyPressed} />;
};
